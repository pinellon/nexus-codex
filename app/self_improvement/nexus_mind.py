...
class NexusMind:
    def __init__(self, api_key: str, auto_mode: bool = False, high_improvement: bool = False, ui_callback=None, restart_mgr=None):
        ...
        # O projeto raiz é 2 niveis acima desta pasta
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "../.."))
        self.sandbox_mgr = SandboxManager(project_root)
        self.score_engine = ScoreEngine(project_root)
...
{{
  ...
  "summary": "Resumo da evolução"
        ...
        # 6. Score inicial
        old_score = self.score_engine.get_last_score()
        self._log(f"📊 Score inicial: {old_score.get('overall', 0)}/100")

        # 7. Prepara o Sandbox
        self._log("🏖️  Criando sandbox para validação segura...")
        sandbox_dir = self.sandbox_mgr.create_sandbox()
        
        # Modifica o caminho para apontar para o sandbox
        action = plan.get("action")
        rel_path = plan.get("file_path")
        
        # Remove barra inicial se houver para não bugar o join
        ...
        if not success:
            ...
            self.sandbox_mgr.discard_sandbox()
            if retry_count < 2:
                self._log("🔄 Retentando com feedback do erro...")
                self._run_one_cycle(user_prompt, retry_count + 1, f"Falha ao gravar arquivo ou aplicar mudança em {rel_path}.")
            return
        
        # 8. Valida (roda pytest no sandbox)
        self._log("🧪 Executando testes de validação no sandbox...")
        tests_passed = self.sandbox_mgr.validate_sandbox()
        
        if not tests_passed:
            self._log("❌ Testes falharam no sandbox! Mudança abortada.")
            self.sandbox_mgr.discard_sandbox()
            if retry_count < 2:
                self._log("🔄 Retentando com feedback do erro dos testes...")
                self._run_one_cycle(user_prompt, retry_count + 1, "Falha nos testes dentro do sandbox. Gere um código que passe nos testes.")
            return

        # 9. Avalia novo Score no sandbox
        self._log("📈 Calculando novo score...")
        # Força apontar score engine para sandbox temporariamente
        self.score_engine.root = sandbox_dir
        new_score = self.score_engine.evaluate_system()
        # Restaura raiz
        self.score_engine.root = self.sandbox_mgr.root_dir
        
        self._log(f"📊 Novo Score: {new_score.get('overall', 0)}/100")
        
        if new_score.get('overall', 0) < old_score.get('overall', 0) - 2.0:
            # Toleramos uma queda mínima de -2.0, mas não mais
            self._log("❌ O novo score é inferior ao anterior. Mudança rejeitada.")
            self.sandbox_mgr.discard_sandbox()
            return

        # 10. Commit e Aplicação no Live System
        self.guard.commit_before_change(f"antes de: {plan.get('summary', 'melhoria')}")
        self._log("🚀 Deploy do Sandbox para o Live System!")
        self.sandbox_mgr.apply_to_live()
        
        # Salva o novo score real
        self.score_engine.evaluate_system()
