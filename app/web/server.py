...
def _module_cards() -> list[dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "status": "online" if not integrations_missing else "ready",
            "tone": "lime" if not integrations_missing else "sky",
            "sector": "Integracoes",
            "icon": "music-4",
            "examples": ["quero ouvir luan santana", "status da casa", "ligar luz da sala"],
            "detail": "Spotify e Home Assistant configurados." if not integrations_missing else f"Integracoes opcionais nao configuradas: {', '.join(integrations_missing)}.",
